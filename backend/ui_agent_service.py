...
class UIAgentService:
    ...
    def _fallback_prompt(self, message: str, workflow_id: str, selected_lora: str) -> str:
        ...
        if workflow_id in {"firered-image-edit", "qwen-rapid-edit-v23"}:
            # Strong edit instruction following the full Image Editing Agent 70-rule protocol
            # Exact execution only, source/character/lighting preservation, precise language,
            # state-aware (clothing/expression/fluids/pose), no unrequested additions.
            return (
                "Edit the source image with absolute precision and zero extra changes. "
                f"User request (execute literally and only this): {request}. "
                "Preserve original lighting direction/intensity/color temperature, camera angle, background, overall quality, and resolution unless the request explicitly says to change them. "
                "For characters: lock face structure, eye color, hair style/color, skin tone, body proportions exactly — only alter the specifically requested elements (clothing state, pose, expression, added fluids/objects, etc.). "
                "Clothing edits must be exact: if removing, reveal precisely what is underneath as described; if adding or adjusting, match the requested fit/tightness/tears/openness. "
                "Use direct, clinical, precise language in the edit instruction (no euphemisms, no 'make sexier'). "
                "Track and reference current visible state if known from conversation (e.g. 'zipper already open, bra visible'). "
                "Output one clear, ready-to-use natural language edit description for the image edit model. No extra commentary."
            )
        identity = ""
        if selected_lora:
